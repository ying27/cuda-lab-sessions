...
*/
  ...
  if (argc != 4) { printf("Usage: ./exe start end step"); exit(0); }
  ...
  for (start=atoi(argv[1]); start < end; start+=step) { 
    ...
    printf("%4.2f",(2.0 * (float) N * (float) N * (float) N) / (1000000.0 * TiempoKernel));  
