...
__global__ void Kernel05(float *g_idata, float *g_odata) {
...

{
  ...
  // Ejecutar el kernel 
  Kernel05<<<nBlocks/2, nThreads>>>(d_v, d_w);
  Kernel05<<<nBlocks/(4*nThreads), nThreads>>>(d_w, d_x);

  // Obtener el resultado parcial desde el host 
  cudaMemcpy(h_x, d_x, numBytesX/4, cudaMemcpyDeviceToHost);
